...
1. 拟合你的线性回归模型 (`lm()`)。
2. 使用 `plot(model)` 或 `ggplot2` 绘制**残差图 (Residuals vs Fitted)**。检查是否有非线性模式或异方差（喇叭形）。
3. 绘制**正态 Q-Q 图**。检查点是否大致落在直线上。
4. (可选) 使用 `shapiro.test(residuals(model))` 进行正态性检验。
5. 如果模型有多个自变量，使用 `vif(model)` 检查**多重共线性**。是否有 VIF \> 5 或 10？
6. 绘制 **Cook 距离图**或计算 `cooks.distance(model)`。是否有 $D_i > 4/n$ 或 \> 0.5 或 \> 1 的点？识别这些强影响点。
7. **总结**你的诊断结果：模型的主要问题是什么？（违反了哪些假设？是否存在共线性？是否有强影响点？）
